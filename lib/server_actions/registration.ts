  ...
  try {
    ...
    if (universityLogoFile instanceof File && universityLogoFile.size > 0) {
      ...
      try {
        ...
        return {
          success: false,
          error: uploadError.message || "Failed to upload university logo.",
        };
      }
    }
    ...
    if (teamLogoFile instanceof File && teamLogoFile.size > 0) {
      ...
      try {
        ...
        return {
          success: false,
          error: uploadError.message || "Failed to upload team logo.",
        };
      }
    }
    ...
    if (missingGameIds.length > 0) {
      return {
        success: false,
        error: "Some players have not created their profiles yet.",
      };
    }
    ...
    const uploadPromises = filesToUpload.map(async ({ file, index, field }) => {
      ...
      if (url) {
        ...
        console.warn(
          `[handleFinalRegistration] Upload promise resolved with null URL for ${file.name}.`
        );
      }
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to upload one or more player files: ${uploadError.message}`,
      };
    }
    ...
    if (
      ...
      console.log("Players Inserted.", playersData);
    } else {
