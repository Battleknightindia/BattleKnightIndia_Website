    ...
    const handleWheel = (e: WheelEvent) => {
      if (carouselRef.current && carouselRef.current.contains(e.target as Node)) {
        ...
        if (e.deltaX > 0 || e.deltaY > 0) {
          ...
          setActiveIndex((prev) => (prev - 1 + TOURNAMENT_DATA.length) % TOURNAMENT_DATA.length)
        }
  ...
  return (
    <div className="w-full bg-black px-4 py-12 md:py-16 relative">
      <motion.div
              ...
              >
                <Badge className="mb-4 bg-blue-500 text-white hover:bg-blue-600 hover:scale-105 transition-all duration-300">Our Past Work</Badge>
                <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4">NorthEast Cup</h2>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {TOURNAMENT_DATA.map((item, index) => {
            ...
            return (
              ...
              >
                <Card className="w-full h-full overflow-hidden rounded-xl border-0 shadow-lg">
                  <img
                    src={item.image}
                    alt={`Slide ${index + 1}`}
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
        {Object.entries(TOURNAMENT_DATA[activeIndex].stats).map(([label, value], index, array) => {
            const colorClass = TOURNAMENT_DATA[activeIndex].statColors?.[
            ...
            const isThirdPanelWhenThreeStats = array.length === 3 && index === 2;

            // Add col-span-2 class for mobile if it's the third panel and there are exactly three stats
            const colSpanClass = isThirdPanelWhenThreeStats ? "col-span-2" : "";

            return (
                <div
                    key={label}
                    className={`${colorClass} p-5 rounded-lg text-white text-center ${colSpanClass}`}
                >
                    <p className="font-semibold text-base md:text-lg capitalize">{label}</p>
