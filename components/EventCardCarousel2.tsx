// ./components/EventCardCarousel.tsx
...
        if (videoId !== id && videoEl && !videoEl.paused) { // Added check for videoEl existence
          videoEl.pause()
...
    } else {
      // Clean up ref when element is null (e.g., during unmount)
...
  // --- FIX: Capture the ref's current value ---
  const currentVideoRefs = videoRefs.current;

  // Pause all videos when component unmounts
  return () => {
    // --- FIX: Use the captured value in the cleanup ---
    Object.values(currentVideoRefs).forEach((video) => {
      // Add a check to ensure video element exists before accessing properties
      if (video && !video.paused) {
        video.pause()
      }
    })
  }
}, []) // Keep the dependency array empty for mount/unmount behavior
...
                  )}
                  playsInline
                  loop // Added loop for videos
                  muted // Added muted to allow autoplay in some browsers
                  onPlay={() => setIsPlaying((prev) => ({ ...prev, [activeItem.id]: true }))}
                  onPause={() => setIsPlaying((prev) => ({ ...prev, [activeItem.id]: false }))}
                  // Optional: add onEnded to reset playing state or loop explicitly
                  // onEnded={() => setIsPlaying((prev) => ({ ...prev, [activeItem.id]: false }))}
                />
                {/* Play/Pause button with higher z-index and pointer-events-auto */}
                <div
                  className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none" // Reduced z-index slightly but still above video
                  onClick={(e) => e.stopPropagation()} // Stop propagation to prevent carousel navigation
                >
                  <button
...
                    onClick={(e) => {
                      e.stopPropagation() // Stop propagation again for the button itself
...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent pt-16 pb-6 px-6 text-white z-10"> {/* Added z-index */}
...
        <div
          className={cn(
            "absolute inset-0 flex items-center justify-between px-4 transition-opacity duration-300 z-20", // Added z-index
...
            {item.type === "image" ? (
              <Image
                src={item.src || "/placeholder.svg"} // FIX: Changed item.image to item.src
                alt={item.title}
                fill
...
            ) : (
              <div className="relative w-full h-full">
                {/* For video thumbnails, you might want to use a placeholder image or a static thumbnail */}
                {/* Using a static placeholder for now */}
...
              </div>
            )}
            {activeIndex === index && <div className="absolute bottom-0 left-0 right-0 h-1 bg-emerald-500 z-50"></div>} {/* Added z-index */}
          </button>
        ))}
      </div>
    </div>
