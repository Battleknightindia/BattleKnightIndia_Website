...
const FormContent = ({}: Record<string, never>): React.ReactElement => {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      setFormData((prevData: RegistrationFormData) => {
        ...
        const allPlayerKeys = Array.from({ length: 7 }, (_, i) =>
          (i + 1).toString()
        );
        ...
        allPlayerKeys.forEach((key) => {
          ...
          newPlayers[key] = {
            ...
            student_id_url:
              prevPlayer?.student_id_url instanceof File
                ? prevPlayer.student_id_url
                : null,
  ...
  const handlePlayerDataChange = async (
      ...
      const newData = {
        ...
        players: {
          ...
          [stateKey]: {
            ...
            ...(field === "student_id_url" && value === null
              ? { student_id_url: null }
              : {}),
  ...
  const handleNextStep = (): void => {
    ...
    switch (activeStep) {
      case 1:
        ...
        if (!validationError && !formData.university.city)
          validationError = "City is required.";
        if (!validationError && !formData.university.state)
          validationError = "State is required.";
        if (!validationError && !formData.university.logo)
          validationError = "University Logo is required.";
        // --- File Size Validation for University Logo ---
        // Check size ONLY if a File object is present
        if (
          !validationError &&
          formData.university.logo instanceof File &&
          formData.university.logo.size > MAX_FILE_SIZE
        ) {
          validationError = `University Logo size exceeds the limit (${
            MAX_FILE_SIZE / 1024
          }KB).`;
        }
      ...
      case 2:
        ...
        if (!validationError && !formData.team.logo)
          validationError = "Team Logo is required.";
        // --- File Size Validation for Team Logo ---
        // Check size ONLY if a File object is present
        if (
          !validationError &&
          formData.team.logo instanceof File &&
          formData.team.logo.size > MAX_FILE_SIZE
        ) {
          validationError = `Team Logo size exceeds the limit (${
            MAX_FILE_SIZE / 1024
          }KB).`;
        }
        // --- End File Size Validation ---
      ...
      case 3:
        ...
        for (let i = 0; i < 5; i++) {
          ...
          // Check size ONLY if a File object is present
          if (
            player.student_id_url instanceof File &&
            player.student_id_url.size > MAX_FILE_SIZE
          ) {
            validationError = `${displayName} Student ID proof size exceeds the limit (${
              MAX_FILE_SIZE / 1024
            }KB).`;
            break; // Exit the loop
          }
        ...
        if (!validationError) {
          ...
          if (substitute) {
            // Check if the substitute object exists
            ...
            const subHasBasicData = requiredSubstituteFields.some(
              (field) =>
            ...
            if (subHasBasicData || subHasFile) {
              ...
              if (subHasBasicData) {
                for (const field of requiredSubstituteFields) {
                  if (!substitute[field as keyof Player]) {
                    validationError = `${field
                      .replace("_", " ")
                      .toUpperCase()} is required for Substitute if any basic information is provided.`;
                    break;
                  }
                }
              }
              ...
              if (
                substitute.student_id_url instanceof File &&
                substitute.student_id_url.size > MAX_FILE_SIZE
              ) {
                validationError = `Substitute Student ID proof size exceeds the limit (${
                  MAX_FILE_SIZE / 1024
                }KB).`;
              }
        ...
        if (!validationError) {
          ...
          if (coach) {
            // Check if the coach object exists
            ...
            const coachHasRequiredBasicData = requiredCoachFields.some(
              (field) =>
                coach[field as keyof Player] !== null &&
                coach[field as keyof Player] !== undefined &&
                coach[field as keyof Player] !== ""
            );
            const coachHasOptionalBasicData = optionalCoachFields.some(
              (field) =>
                coach[field as keyof Player] !== null &&
                coach[field as keyof Player] !== undefined &&
                coach[field as keyof Player] !== ""
            ...
            if (
              coachHasRequiredBasicData ||
              coachHasOptionalBasicData ||
              coachHasFile
            ) {
              ...
              if (coachHasRequiredBasicData || coachHasOptionalBasicData) {
                for (const field of requiredCoachFields) {
                  if (!coach[field as keyof Player]) {
                    validationError = `${field
                      .replace("_", " ")
                      .toUpperCase()} is required for Coach if any information is provided.`;
                    break;
                  }
                }
              }
              ...
              // Check size ONLY if a file is actually present - FIX APPLIED HERE
              if (
                coach.student_id_url instanceof File &&
                coach.student_id_url.size > MAX_FILE_SIZE
              ) {
                validationError = `Coach ID proof size exceeds the limit (${
                  MAX_FILE_SIZE / 1024
                }KB).`;
              }
              if (validationError) break; // Exit if file size is too large
  ...
  const handleFinalSubmit = async (
    ...
    if (
      ...
      finalValidationErrors = `University Logo size exceeds the limit (${
        MAX_FILE_SIZE / 1024
      }KB). Please go back to Step 1.`;
    }

    // Check step 2 validity (presence and type)
    if (
      !finalValidationErrors &&
      (!formData.team.name || !(formData.team.logo instanceof File))
    ) {
    ...
    if (
      ...
      finalValidationErrors = `Team Logo size exceeds the limit (${
        MAX_FILE_SIZE / 1024
      }KB). Please go back to Step 2.`;
    }

    // Check step 3 (basic check for main players)
    const mainPlayersFilled = [1, 2, 3, 4, 5].every(
      (index) => formData.players[index.toString()]?.name
    );
    if (!finalValidationErrors && !mainPlayersFilled) {
      finalValidationErrors =
        "Please complete Step 3 (Player Details) for at least the main 5 players.";
    }
    ...
    if (!finalValidationErrors) {
      for (let i = 0; i < 5; i++) {
        ...
        if (
          ...
          finalValidationErrors = `${displayName} Student ID proof size exceeds the limit (${
            MAX_FILE_SIZE / 1024
          }KB). Please go back to Step 3.`;
          break;
        }
      }
    }
    ...
    if (!finalValidationErrors) {
      ...
      for (const key of optionalPlayerKeys) {
        ...
        if (
          ...
          finalValidationErrors = `${displayName} ID proof size exceeds the limit (${
            MAX_FILE_SIZE / 1024
          }KB). Please go back to Step 3.`;
          break;
        }
    ...
    try {
      ...
      const playerFiles = Object.entries(formData.players)
        ...
        .filter(
          (entry): entry is [string, Player] =>
            entry[1] !== undefined &&
            entry[1]?.student_id_url instanceof File
        )
  ...
  const renderStepComponent = (): React.ReactNode => {
    switch (activeStep) {
      ...
      case 3:
        return (
          <Step3 data={formData.players} onDataChange={handlePlayerDataChange} />
        );
      case 4:
        return (
          <Step4
        ...
        <CardContent className="flex flex-col gap-6 overflow-hidden">
          ...
          <div className="flex justify-between mt-6">
            <Button
              type="button"
              onClick={() =>
                setActiveStep((prev: number) => Math.max(1, prev - 1))
              }
